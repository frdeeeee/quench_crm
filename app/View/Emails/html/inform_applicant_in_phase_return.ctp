<p><?php echo  $data['Enquiry']['name']; ?>，您好:</p>
恭喜顺利完成<?php echo  $data['Project']['name']; ?>，欢迎回到祖国！
<p>请在优势后台（<?php echo Router::url(array('controller'=>'Students','action'=>'login'),TRUE); ?>）上登记回国，
并上传护照页入境章和雇主评估表，以便优势办向外方确认你的回国信息，并开具实习报告。同时请提交赴美感言和照片，发送至邮箱<?php echo $data['User']['username'];?>。
优势办会以同学们每次Check-In情况、感言和照片为参考，评选<?php echo date('Y',time()); ?>年优秀学生。</p>
<hr />
<ul>
	<li>您的登陆用户名是您目前使用的电子邮件：<?php echo $data['Enquiry']['email'];  ?></li>
	<li>您的登陆口令是您的手机号码: <?php echo $data['Enquiry']['mobile'];  ?></li>
</ul>
<p>如有任何问题，欢迎您随时通过电话或者电子邮件<?php echo $data['User']['username'];?>与我联系，我将为您提供最优质的服务。</p>
<p>谢谢！</p>
<hr>
<p>优势项目办公室: <?php echo $data['User']['name'];?></p>
<p>地址：北京市朝阳区西大望路15号外企大厦B座1802B 100022</p>
<p>电话：4006-501-801</p>