...
$.fn.sliderNav = function(options) {
	...
	var slider = $(this);
	$(slider).addClass('list_slider');
	$('.slider-content li:first', slider).addClass('selected');
	$(slider).append('<div class="slider-nav"><ul></ul></div>');
	for ( var i in o.items)
		$('.slider-nav ul', slider).append(
				"<li><a alt='#" + o.items[i] + "'>" + o.items[i] + "</a></li>");
	var height = $('.slider-nav', slider).height();
	if (o.height)
		height = o.height;
	$('.slider-content', slider).css('height', height);
	$('.slider-nav', slider).css('height', height+20);
	if (o.debug)
	...
	if (true) {
		...
		$(slider)
				.prepend(
						'<div class="slide-up end">UP<span class="arrow up"></span></div>');
		$(slider)
				.append(
						'<div class="slide-down">DOWN<span class="arrow down"></span></div>');
